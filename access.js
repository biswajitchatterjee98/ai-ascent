(function (global) {
  ...
  function loadPermissions() {
    ...
    return global.AscentApi.permissions(global.AscentAuth.currentToken()).then(function (result) {
      if (!result.ok) {
  ...
  function sectionTitle(sectionId) {
    ...
    var el = document.querySelector('.content-block[data-section-id="' + sectionId + '"] h2');
    if (!el) el = document.getElementById(sectionId);
  ...
  function wireModal() {
    ...
    document.getElementById('access-request-form').addEventListener('submit', function (e) {
      ...
      global.AscentApi.requestAccess(global.AscentAuth.currentToken(), sectionId, notes).then(function (result) {
        if (!result.ok) {
  ...
  function showAdminLink() {
    ...
    if (adminLink && global.AscentAuth.isAdmin()) adminLink.classList.add('is-visible');
    global.AscentAuth.fillUserChrome();
  }
