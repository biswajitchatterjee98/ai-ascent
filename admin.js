(function () {
  var LEARNER_USERS = [
    'learner01', 'learner02', 'learner03', 'trainer', 'guest',
    'opslead', 'analyst', 'hrdesk', 'demo'
  ];
  ...
  function needsApi(action) {
    if (AscentApi.configured()) return false;
  ...
  document.getElementById('policy-form').addEventListener('submit', function (e) {
    ...
    AscentApi.adminSetPolicy(
      AscentAuth.currentToken(),
      sectionId,
  ...
  document.addEventListener('click', function (e) {
    ...
    if (approve) {
      ...
      AscentApi.adminReviewRequest(AscentAuth.currentToken(), approve, 'approve', '', '').then(function (result) {
    ...
    if (reject) {
      ...
      AscentApi.adminReviewRequest(AscentAuth.currentToken(), reject, 'reject', '', 'Rejected by admin').then(function (result) {
    ...
    if (revoke) {
      ...
      AscentApi.adminRevoke(AscentAuth.currentToken(), revoke).then(function (result) {
    ...
    if (extend) {
      ...
      AscentApi.adminExtend(AscentAuth.currentToken(), extend, expiry).then(function (result) {
